...
export const LuxuryProjectsShowcase: React.FC<{ onOpenInquiry: () => void }> = ({ onOpenInquiry }) => {
  ...
  return (
    <section id="projects" className="py-20 sm:py-24 bg-[#E4EBF1] relative overflow-hidden border-t border-white/60">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="flex flex-col md:flex-row md:items-end justify-between gap-6 mb-12">
          ...
          >
            ...
            <p className="text-xs sm:text-sm text-[#11172C]/80 font-medium mt-2 max-w-xl">
              اجرای سازه‌های شیشه‌ای و ورودی‌های مدرن در مناطق ۱ تا ۵ تهران
            </p>
          </motion.div>
          ...
          <motion.div
            ...
            className="flex flex-wrap items-center gap-1.5 p-1 bg-[#CBD8E2]/80 backdrop-blur-md rounded-2xl border border-white/80 shadow-xs"
          >
            {filters.map((tab) => {
        ...
        {/* 3-Column Architectural Project Gallery Layout */}
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 lg:gap-8">
          {storeProjects.slice(0, 2).map((project, index) => (
            <motion.div
              ...
              className="group relative rounded-3xl overflow-hidden bg-slate-900 border border-white/80 shadow-md hover:shadow-2xl transition-all duration-500 cursor-pointer aspect-[16/12]"
          ...
          <motion.a
            ...
            className="group relative rounded-3xl overflow-hidden bg-slate-950 border border-white/80 hover:border-white shadow-lg hover:shadow-2xl transition-all duration-500 hover:-translate-y-1.5 cursor-pointer aspect-[16/12] flex flex-col justify-between p-6 sm:p-7"
          >
            ...
            <div className="relative z-10 mt-auto">
              ...
              <p className="text-xs text-slate-300 font-medium mt-1.5 leading-relaxed line-clamp-2">
                مشاهده گالری کامل پروژه‌ها در مناطق ۱ تا ۵ تهران، دانشگاه‌ها، هتل‌ها و برج‌های فاخر
              </p>
        ...
        >
          <div className="inline-flex flex-col sm:flex-row items-center gap-3 sm:gap-4 p-3 sm:p-3.5 rounded-2xl bg-[#CBD8E2]/80 backdrop-blur-md border border-white/80 shadow-xs">
            <span className="text-xs font-bold text-[#06080F] px-3">
              بیش از ۸۵۰ پروژه اجرایی فاخر در مناطق ۱ تا ۵ تهران، دانشگاه‌ها، بیمارستان‌ها و هتل‌های شاخص
            </span>
            <div className="flex flex-wrap items-center gap-2.5 w-full sm:w-auto">
              <button
                onClick={onOpenInquiry}
                id="btn-projects-inquiry"
                className="w-full sm:w-auto px-5 py-2.5 rounded-full bg-[#00F090] hover:bg-[#00D882] text-[#06080F] text-xs sm:text-sm font-black flex items-center justify-center gap-1.5 shadow-md shadow-[#00F090]/20 transition-all active:scale-[0.98] cursor-pointer"
              >
                <Sparkles className="w-3.5 h-3.5 text-[#06080F]" />
