...
export const ArticlesSection: React.FC = () => {
  ...
  return (
    <section id="articles" className="py-20 bg-[#E4EBF1] backdrop-blur-md border-b border-white/60">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        >
          <div>
            <span className="text-xs font-bold text-[#11172C] bg-[#CBD8E2]/80 border border-white/80 px-3.5 py-1 rounded-full shadow-xs inline-flex items-center gap-1.5">
              <BookOpen className="w-3.5 h-3.5 text-[#06080F]" />
              دانشنامه و مقالات تخصصی
            </span>
            <h2 className="text-2xl sm:text-3xl font-black text-[#06080F] mt-3">
              مقالات و راهنماهای مهندسی
            </h2>
            <p className="text-xs sm:text-sm text-[#11172C]/80 mt-2 max-w-2xl leading-relaxed">
              جدیدترین مطالب آموزشی، استانداردهای روز دنیا و تحلیل‌های فنی درب‌های اتوماتیک و سازه‌های شیشه‌ای.
            </p>
          </div>

          <div className="flex items-center gap-2">
            <span className="text-xs font-bold text-[#11172C]/70 hidden sm:inline">
              ۵ راهنمای مرجع تخصصی سازه‌های شیشه‌ای و درب اتوماتیک
            </span>
          </div>
        </motion.div>

        {/* Articles Grid with Frosted Glass Cards (2 Featured Articles + 1 Glassmorphic Archive Preview Card) */}
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {articles.slice(0, 2).map((article, idx) => (
            <motion.a
              key={article.id}
              href={`blog.html?id=${article.id}`}
              initial={{ opacity: 0, y: 25 }}
              whileInView={{ opacity: 1, y: 0 }}
              viewport={{ once: true, margin: '-40px' }}
              transition={{ duration: 0.5, delay: idx * 0.1, ease: [0.16, 1, 0.3, 1] }}
              className="p-4 rounded-2xl bg-[#CBD8E2]/60 backdrop-blur-xl border border-white/80 hover:bg-[#CBD8E2]/90 shadow-xs hover:shadow-md transition-all duration-300 group flex flex-col justify-between cursor-pointer"
            >
              <div>
                {/* Article Thumbnail */}
          ...
          {/* 3rd Card: Premium Architectural Photo Knowledge Archive Card */}
          <motion.a
            href="blog.html"
            ...
            className="group relative rounded-3xl overflow-hidden bg-slate-950 border border-white/80 hover:border-white shadow-lg hover:shadow-2xl transition-all duration-500 hover:-translate-y-1.5 cursor-pointer min-h-[380px] flex flex-col justify-between p-6 sm:p-7"
          >
        ...
        >
          <a
            href="blog.html"
            id="btn-blog-archive"
            className="inline-flex items-center gap-2 px-6 py-2.5 rounded-full bg-[#06080F] hover:bg-[#11172C] text-[#00F090] font-bold text-xs sm:text-sm border border-[#00F090]/40 hover:border-[#00F090] shadow-[0_0_12px_rgba(0,240,144,0.12)] hover:shadow-[0_0_16px_rgba(0,240,144,0.25)] transition-all active:scale-[0.98] cursor-pointer group"
          >
            <BookOpen className="w-4 h-4 text-[#00F090]" />
            <span>مشاهده دانشنامه و مقالات تخصصی</span>
            <ArrowLeft className="w-4 h-4 text-[#00F090] group-hover:-translate-x-1 transition-transform" />
          </a>
        </motion.div>
