...
export const ArchitecturalServiceCard: React.FC<ArchitecturalServiceCardProps> = ({
  ...
  const persianIndex = (index + 1).toLocaleString('fa-IR', { minimumIntegerDigits: 2 });
  ...
  return (
    ...
    >
      {/* 
        1. IMAGE SECTION (Exactly 70% height)
        - Completely fills the 70% container using object-fit: cover
      */}
      <div className="relative w-full h-[70%] overflow-hidden bg-[#0A0F1D]">
        {/* Top-Right Index Badge */}
        <div className="absolute top-3 right-3 sm:top-4 sm:right-4 z-10 flex items-center gap-1.5 px-2.5 py-1 rounded-lg bg-[#06080F]/85 border border-white/10 shadow-sm backdrop-blur-md">
          <span className="w-1.5 h-1.5 rounded-full bg-[#00F5A0] shadow-[0_0_6px_#00F5A0]" />
          <span className="text-white/90 text-[11px] sm:text-xs font-mono font-bold tracking-wider">
            {persianIndex}
          </span>
        </div>
        ...
        <div 
          className="absolute top-3 left-3 sm:top-4 sm:left-4 z-10 w-7 h-7 sm:w-8 sm:h-8 rounded-lg bg-[#06080F]/85 border border-white/10 shadow-sm backdrop-blur-md flex items-center justify-center"
          title={data.titleFa}
        >
      ...
      {/* 
        2. SERVICE TITLE SECTION (Remaining 30% height)
        - Persian title
        - English subtitle
        - Inquiry action
      */}
      <div className="relative w-full h-[30%] flex flex-col justify-between px-3.5 sm:px-5 py-2.5 sm:py-3 bg-gradient-to-b from-[#090E1A] to-[#04060B] border-t border-white/10">
        <div>
          ...
          <p className="text-[#CBD8E2]/70 font-mono text-[8.5px] sm:text-[9.5px] md:text-[10.5px] uppercase tracking-wider font-semibold mt-0.5 line-clamp-1">
            {data.titleEn}
          </p>
        </div>
        ...
        <button
          type="button"
          onClick={handleClick}
          className="inline-flex items-center justify-between w-full pt-1.5 border-t border-white/[0.08] text-[10.5px] sm:text-xs font-bold text-white/80 hover:text-[#00F5A0] transition-colors duration-200 cursor-pointer group/action focus:outline-none"
          aria-label={`استعلام قیمت ${data.titleFa}`}
        >
          <span className="inline-flex items-center gap-1.5">
            <Calculator className="w-3.5 h-3.5 text-[#00F5A0] shrink-0" />
            <span className="select-none">استعلام و محاسبه آنلاین</span>
          </span>

          <span className="w-5 h-5 sm:w-6 sm:h-6 rounded-full bg-white/10 group-hover/action:bg-[#00F5A0] group-hover/action:text-[#06080F] text-white flex items-center justify-center transition-all duration-300 shrink-0">
